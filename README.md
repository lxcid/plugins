...
claude plugin install intentloop@lxcid

# Codex
codex plugin marketplace add lxcid/intentloop-plugin
codex plugin add intentloop@lxcid
```
...
Claude Code can also load the package for a single development session with `claude --plugin-dir ./plugins/intentloop`.
...
claude plugin update intentloop@lxcid
...
Start a new session after updating. To uninstall, run `claude plugin uninstall intentloop@lxcid` or `codex plugin remove intentloop@lxcid`.

See the [package README](plugins/intentloop/README.md) for manifest versioning and where shared workflow content will live. Packaging references: [OpenAI plugins](https://developers.openai.com/plugins/build/plugins), [Claude Code plugins](https://code.claude.com/docs/en/plugins), and [Claude Code marketplaces](https://code.claude.com/docs/en/plugin-marketplaces).
