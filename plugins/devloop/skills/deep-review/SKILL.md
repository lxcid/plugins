...
1. Resolve the target before reviewing. Honor an explicit target. If the request is missing a target or is ambiguous and this session has already reviewed a PR, branch, or worktree, first ask whether to continue reviewing that target and wait for an answer. If the user agrees, continue there without another target-selection question. Only if they decline without naming a replacement, or there is no previous review target, inspect recently updated open PRs, recently active local branches with changes, and worktrees (`git worktree list`). Present a short list with PR titles/numbers, branches, worktree paths, and available recency/change information. Ask the user which target to review and wait for an answer; do not select one merely because it is current or newest.
