---
name: adopt
description: Install devloop's engineering patterns and pipeline process into a repository, and update them safely on re-run. Use when the user asks to adopt, install, or set up devloop patterns, conventions, or pipelines in a project, to add an engineering-practice section such as design judgment or commit conventions to AGENTS.md, to set up intent, spec, and plan pipelines under docs/pipelines/, or to refresh previously adopted patterns to their latest version. Accepts one or more pattern ids, `all`, or no argument to choose from the catalog.
disable-model-invocation: true
---

# Adopt

Install one or more patterns into the target repository, and keep previously installed ones current without destroying local edits. Most patterns are one section of `AGENTS.md`; a pattern can also install a document of its own.

Each reference file under `references/` installs one managed section. Its YAML frontmatter carries the pattern's `id`, the file's `version`, the `section` heading, and optionally a `target` path from the repository root, which defaults to `AGENTS.md`. Everything below the frontmatter is the text installed into the target. A pattern is every reference file sharing its id: `references/<id>.md` holds its `AGENTS.md` section, and `references/<id>-<name>.md` holds any other file it installs. Edit a reference file to change what it installs, and raise that file's `version` when you change one that has already shipped.
...
1. Never edit a managed section's text in place in its target file. Change the reference file and re-run, so the installed copy and its recorded baseline stay consistent.
...
Each reference file names its target, and `AGENTS.md` at the repository root is the default. If `AGENTS.md` does not exist, create it with an `# Agent Guide` heading and a one-line statement that it holds instructions for coding agents in this repository. If any other target does not exist, create it, along with any missing parent directories, holding only its managed section.
...
## Phase 3: Decide, Per Section

Decide each reference file's section on its own, in its own target. A pattern with two sections can have two different outcomes.
...
Compute the installed body's hash with the same trim and digest used for the baseline in Phase 4. The awk program spells the current line `$(0)`, because Claude Code replaces a dollar sign followed by a digit with the skill's arguments. Here, `target` is the reference file's target path:

```bash
# installed body
awk -v id=design-judgment '$(0) ~ "^<!-- devloop:" id " "{f=1;next} $(0) ~ "^<!-- /devloop:" id " -->$"{f=0} f' "$target" | perl -0777 -pe 's/\A\s+|\s+\z//g' | shasum -a 256 | cut -c1-8
```
...
If the target already contains a heading matching the reference's `section` but no devloop markers for it, do not add a second one. Show the existing section, say which parts the pattern would add, and ask whether to replace it, merge into it, or skip.

## Phase 4: Apply

For each section being written, take its reference body, format it if the project has a Markdown formatter, and hash the result. That is the baseline, and it goes in the marker.

Format the body on its own — write it to a temporary file, run the project's formatter on that file, and read it back. Do not run a formatter over the target file. Reformatting the whole document rewrites the user's own prose and can normalize a customized section back into something that hashes as untouched, which silently converts local content into replaceable content.
...
List each requested pattern and the outcome of each of its sections: added, updated (with the version change), current, locally modified, or skipped with the reason. Name any conflict the user resolved and how, and say plainly when a section was left carrying local content.
