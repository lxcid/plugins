...
**Fail loud > self-heal.** Prefer a single attempt that exposes failure and gives the operator a useful recovery hint. Don't add retry loops, sequencing tricks, or duplicate-execution guards merely to make the operation appear seamless. Add automatic recovery only when a present requirement justifies its extra states and complexity.
